...
local blips = {
	{title="Mine", colour=0, id=357, x=-593.95855712891, y=2091.75390625, z=131.71856689453},
  }
...
-- 	shopCostMenu = 100
-- 	TriggerServerEvent("buyBread:minerJob", shopCostMenu)
...
AddEventHandler("showReward:minerJob", function(completReward)
	TriggerEvent('notifications', "#29c501", "MINER", "You become "..completReward.."$")
end)
		...
		if delivery_point_distance < 500.0 then
			...
			if delivery_point_distance < 1.5 then
				if isMining == false then
					DisplayHelpText("Press ~INPUT_CONTEXT~ to ~r~mining")
				

					if(IsControlJustReleased(1, 38))then
						QBCore.Functions.TriggerCallback('MW-minerjob:server:GetItem', function(hasItem)
							if hasItem then
							mining()
							TriggerServerEvent("removePickaxe:minerJob")
						else
							QBCore.Functions.Notify('Pickaxe not found.', 'error')
						end
					end, "pickaxe")
    ...
    Citizen.CreateThread(function()
        while impacts < 3 do
            Citizen.Wait(1)
				...
				QBCore.Functions.Progressbar("MINING", "Mining ...", 10000, false, true, {
